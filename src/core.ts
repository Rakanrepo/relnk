 ...
 * });
 */
export async function observe(payload: ObservePayload): Promise<void> {
  ...
  const url = `${RELNK_API_BASE}/v1/observe`;
  try {
...
export async function complete(options: CompleteOptions): Promise<CompleteResult> {
  ...
  const url = `${RELNK_API_BASE}/v1/complete`;
  const res = await fetch(url, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
  ...
  if (res.status === 200) {
    const body = (await res.json()) as { forward?: boolean; [key: string]: unknown };
    if (body.forward === true) {
      return { forward: true };
    }
    return { completion: body } as CompleteResultCompletion;
  }
